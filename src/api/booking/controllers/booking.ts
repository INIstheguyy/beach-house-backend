...
export default factories.createCoreController('api::booking.booking', ({ strapi }) => ({
  ...
  async initializeBooking(ctx) {
    ...
    try {
      ...
      const agentCommission = Math.round((totalAmount * commissionRate) / 100);
      ...
      const booking: any = await strapi.entityService.create('api::booking.booking', {
        data: {
      ...
      try {
        ...
        strapi.log.error('Flutterwave initialization failed:', error.response?.data || error.message);
  ...
  async verifyPayment(ctx) {
    ...
    try {
      ...
      if (paymentData.status === 'successful' && paymentData.tx_ref === txRefStr) {
        // Find booking by reference
        const bookings: any = await strapi.entityService.findMany('api::booking.booking', {
      ...
      strapi.log.error('Payment verification error:', error.response?.data || error.message);
